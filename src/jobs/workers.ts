...
export const startWorkers = (models: {
  ...
  if (connection) {
    ...
    trendingWorker.on('failed', (job, err) => logger.error('Trending worker failed', { jobId: job?.id, err: err.message }));
    allWorkers.push(trendingWorker);
  }
  ...
  if (connection) {
    const notificationWorker = new Worker(
      ...
      async (job) => {
        ...
        const docs = userIds.map((userId: string) => ({ userId, type, title, body, referenceId }));
    ...
    notificationWorker.on('failed', (job, err) => logger.error('Notification worker failed', { jobId: job?.id, err: err.message }));
    allWorkers.push(notificationWorker);
  }
  ...
  if (connection) {
    const paymentWorker = new Worker(
      ...
      async (job) => {
        ...
        if (!success) throw new Error('PhonePe refund failed');

        await models.marketplaceOrderModel.findByIdAndUpdate(orderId, { status: 'refunded', refundTransactionId: refundId });
        logger.info('Refund successful and order updated', { orderId });
      },
      { connection, concurrency: 2 }
    );
    paymentWorker.on('failed', (job, err) => logger.error('Payment worker failed', { jobId: job?.id, err: err.message }));
    allWorkers.push(paymentWorker);
  }
  ...
  if (connection) {
    const presenceWorker = new Worker(
      ...
      async () => {
        ...
        try {
          await pool.query(
            `UPDATE users SET last_active_at = NOW() WHERE id = ANY($1::uuid[])`,
            [userIds]
          );
    ...
    presenceWorker.on('failed', (job, err) => logger.error('Presence worker failed', { err: (err as any).message }));
    allWorkers.push(presenceWorker);
  }
  ...
  if (connection) {
    ...
    subscriptionWorker.on('failed', (job, err) => logger.error('Subscription worker failed', { jobId: job?.id, err: err.message }));
    allWorkers.push(subscriptionWorker);
  }

  if (allWorkers.length > 0) {
    logger.info(`✌️ BullMQ workers started (${allWorkers.length})`);
  } else {
